...
public class AddToCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (quantity != 0) {
                quantity = c.getQuantityProductInCartByProductId(user.getUserId(), productId) + 1;
                c.updateQuantityProductInCart(user.getUserId(), productId, quantity);
            } else {
                quantity = 1;
                c.addProductToCart(user.getUserId(), productId, quantity);
            }
